...
#endif
 protected:
  /// Per-pixel draw with rows (true) or columns (false) as the outer loop.
  template<bool ROWS_OUTER>
  void draw_(int x, int y, display::Display *display, Color color_on, Color color_off, int img_x0, int img_y0, int w,
             int h);
